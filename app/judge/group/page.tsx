...
export default function GroupScoringPage() {
    ...
    async function load() {
      try {
        ...
        const results = await Promise.all(
          regIds.map((id) =>
            API.get(
              `/registrations/${id}?populate[user]=*&populate[championship]=*&populate[age_category]=*`,
              {
        ...
        if (regsData.length > 0) {
          const first = regsData[0];
          setMeta({
            championship:
              first.attributes.championship.data.attributes.name,
            ageCategory:
              first.attributes.age_category.data.attributes.name,
          });
        }
  ...
  async function handleSubmitAll() {
    ...
    // validation: all asanas filled
    for (const id of regIds) {
      const s = scores[id];
      if (!s || !s.asana1 || !s.asana2 || !s.asana3 || !s.asana4) {
        addToast(
          "Please fill all 4 asana scores for every candidate.",
          "error"
        );
        return;
      }
  ...
  return (
    ...
    >
      {/* HEADER INFO LIKE YOUR SHEET */}
      {user && meta && (
        <div className="mb-5 bg-white/70 backdrop-blur-md border border-white/80 rounded-2xl shadow p-4 text-sm">
          <p>
        ...
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
            {regs.map((reg, index) => {
              ...
              const name =
                reg.attributes.user.data.attributes.username;
